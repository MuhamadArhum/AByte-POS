...
const PaymentVoucherModal = ({ isOpen, onClose, onSuccess }: any) => {
  ...
  useEffect(() => {
    if (isOpen) {
      api.get('/accounting/accounts', { params: { type: 'expense', limit: 200 } })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.voucher_date || !formData.payment_to || !formData.account_id || !formData.amount) {
    ...
    setLoading(true);
    try {
      await api.post('/accounting/payment-vouchers', formData);
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={formData.voucher_date}
                ...
                className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Payment To *</label>
              <input type="text" value={formData.payment_to}
                ...
                placeholder="e.g., ABC Suppliers" required />
            </div>
            <div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button type="submit" disabled={loading}
              className="flex-1 bg-red-600 text-white px-6 py-3 rounded-xl hover:bg-red-700 font-medium transition disabled:bg-gray-400">
...
const PaymentVouchers = () => {
  ...
  const exportToCSV = () => {
    const headers = ['Voucher #', 'Date', 'Payment To', 'Type', 'Amount', 'Method', 'Account', 'Created By'];
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setShowModal(true)}
            className="flex items-center gap-2 bg-red-600 text-white px-6 py-3 rounded-xl hover:bg-red-700 transition shadow-lg">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No Payment Vouchers</h3>
          <p className="text-gray-500 mb-6">Start by creating your first payment voucher</p>
          <button onClick={() => setShowModal(true)}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Payment To</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {vouchers.map((voucher) => (
                    <tr key={voucher.voucher_id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-800">{voucher.payment_to}</td>
                      <td className="px-6 py-4">
                        <span className="px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700 capitalize">
                          {voucher.payment_type}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className="font-semibold text-red-600">Rs. {Number(voucher.amount).toFixed(2)}</span>
                      </td>
