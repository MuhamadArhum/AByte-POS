...
import { FileText, Plus, Trash2, Download, Search, ChevronDown, X } from 'lucide-react';
...
const AccountSelector = ({
  value, onChange, accounts, onAfterSelect, accentClass,
}: {
  value: string;
  ...
  const ring = accentClass ?? 'focus:ring-red-500';

  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)}
        className={`w-full flex items-center justify-between px-2 py-1.5 border border-gray-200 rounded text-sm bg-white hover:border-red-400 focus:outline-none focus:ring-2 ${ring} text-left`}>
              ...
              : filtered.map((a, idx) => (
                <li key={a.account_id}>
                  <button type="button" onClick={() => select(String(a.account_id))}
                    className={`w-full text-left px-3 py-2 flex items-center gap-2 text-sm ${idx === hi ? 'bg-red-50' : 'hover:bg-gray-50'}`}>
            ...
            <tbody>
              ...
              {savedLines.map((line, i) => (
                <tr key={line.voucher_id} className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'}`}>
                  <td className="px-3 py-2.5 font-mono text-xs font-bold text-red-600">{line.voucher_number}</td>
              ...
              {/* Active input row */}
              <tr className="border-b-2 border-red-300 bg-red-50/40">
                ...
                <td className="px-2 py-2">
                  <input
                    ...
                    className="w-full px-2 py-1.5 border border-gray-200 rounded text-sm focus:ring-2 focus:ring-red-500 bg-white outline-none" />
                </td>
                <td className="px-2 py-2">
                  <input
                    ref={amountRef}
                    ...
                    placeholder="0.00"
                    className="w-full px-2 py-1.5 border border-red-200 bg-red-50 rounded text-sm text-right font-semibold text-red-700 focus:ring-2 focus:ring-red-500 outline-none" />
                </td>
                <td />
              </tr>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">
              Cash Payment Voucher <span className="text-red-600">(CPV)</span>
            </h1>
            <p className="text-sm text-gray-500">Track all outgoing payments</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-5 py-2 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition shadow-md">
            <Plus size={17} /> New CPV
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16">
            <div className="animate-spin h-7 w-7 rounded-full border-2 border-red-600 border-t-transparent" />
          </div>
        ) : vouchers.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <FileText size={44} className="mx-auto mb-3 opacity-20" />
            <p className="font-medium">No Cash Payment Vouchers found</p>
            <p className="text-sm mt-1">Click <strong>New CPV</strong> to create entries</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {vouchers.map((v, i) => (
                    <tr key={v.voucher_id}
                      className={`border-b border-gray-50 hover:bg-red-50/30 transition ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                      <td className="px-4 py-3 font-mono text-xs font-bold text-red-600">{v.voucher_number}</td>
                      ...
                      <td className="px-4 py-3 text-gray-500">{v.description || v.payment_to || '—'}</td>
