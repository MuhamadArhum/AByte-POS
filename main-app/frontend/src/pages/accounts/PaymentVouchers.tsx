...
import { ArrowUpCircle, Plus, Trash2, Download, Search, ChevronDown, ArrowLeft } from 'lucide-react';
...
const CPVForm = ({ onBack, onRefresh }: { onBack: () => void; onRefresh: () => void }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Top Bar */}
      <div className="bg-gradient-to-r from-rose-600 to-red-700 px-6 py-4 shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-lg leading-tight">
                  New Cash Payment Voucher <span className="text-rose-200 font-normal text-sm">(CPV)</span>
                </h2>
                <p className="text-rose-100 text-xs">Press Enter on Amount to save each line</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      {/* Entry Form */}
      <div className="px-6 py-5 bg-rose-50/60 border-b border-rose-100 shrink-0">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={saveEntry} disabled={saving}
                className="px-4 py-2 bg-rose-600 text-white rounded-lg text-sm font-semibold hover:bg-rose-700 transition disabled:opacity-50 flex items-center gap-1.5">
                {saving
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {savedLines.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <ArrowUpCircle size={40} className="mx-auto mb-3 opacity-20" />
              ...
              <tbody>
                {savedLines.map((line, i) => (
                  <tr key={line.voucher_id} className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                    <td className="px-5 py-3.5 font-mono text-xs font-bold text-rose-600">{line.voucher_number}</td>
                    ...
                    <td className="px-5 py-3.5 text-right font-semibold text-rose-700 font-mono">
...
const PaymentVouchers = () => {
  ...
  // Full-page form view
  if (view === 'new') {
    return <CPVForm onBack={() => setView('list')} onRefresh={fetchVouchers} />;
  }
